...
public final class DBOperations {
    ...
    //Operations of  Salary
    public Cursor getSalarys(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.SALARY);
        return  database.rawQuery(sql, null);
    }

    public Cursor getSalaryById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                OrderDB.Tables.SALARY,
                OrderContract.Salarys.ID);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }
    ...
    public String insertSalary(Salary salary){
        SQLiteDatabase database = db.getWritableDatabase();
        String idSalary = OrderContract.Salarys.generateIdSalary();
        ContentValues values = new ContentValues();
        values.put(OrderContract.Salarys.ID, idSalary);
        values.put(OrderContract.Salarys.EMP_NO, salary.getEmpNo());
        values.put(OrderContract.Salarys.SALARY, salary.getSalary());
        values.put(OrderContract.Salarys.FROMDATE, salary.getFromDate());
        database.insertOrThrow(OrderDB.Tables.SALARY, null, values);
        return idSalary;
    }
    ...
    public boolean updateSalary(Salary salary){
        ...
        values.put(OrderContract.Salarys.SALARY, salary.getSalary());
        values.put(OrderContract.Salarys.FROMDATE, salary.getFromDate());
        String whereClause = String.format("%s=?", OrderContract.Salarys.ID);
        String[] whereArgs = {salary.getIdSalary()};
        int result = database.update(OrderDB.Tables.SALARY,
                values, whereClause, whereArgs);
        return result>0;
    }

    public boolean deleteSalary(String idSalary){
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = OrderContract.Salarys.ID + "=?";
        String[] whereArgs = {idSalary};
        int result =  database.delete(OrderDB.Tables.SALARY,
                whereClause, whereArgs);
        return result > 0;
    }
